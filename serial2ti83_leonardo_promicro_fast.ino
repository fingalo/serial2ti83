...
const int TIP = 3; //arduino port where the tip of the stereo jack plug is connected
const int RING = 2; //arduino port where the middle part of the stereo jack plug is connected
const unsigned long TXTIMEOUT = 1000; //ms
const unsigned long RXTIMEOUT = 1000; //ms

#define TI_TIP_IS_HIGH   (PIND & (uint8_t)0b00000001) // No need for bitshift because it's the first pin.
#define TI_RING_IS_HIGH ((PIND & (uint8_t)0b00000010) >> 1)

#define TI_TIP_IS_LOW   ((~PIND) & (uint8_t)0b00000001) // No need for bitshift because it's the first pin.
#define TI_RING_IS_LOW (((~PIND) & (uint8_t)0b00000010) >> 1)

inline void TiTipLow()
{
    PORTD &= (uint8_t)0b11111110; // Save an instruction https://www.nongnu.org/avr-libc/user-manual/FAQ.html#faq_intpromote
}
...
{
    PORTD |= (uint8_t)0b00000001;
}
inline void TiRingHigh()
{
    PORTD |= (uint8_t)0b00000010;
}
inline void TiTipOutput()
{
    DDRD |= (uint8_t)0b00000001;
}
inline void TiRingOutput()
{
    DDRD |= (uint8_t)0b00000010;
}
inline void TiTipInput()
{
    DDRD &= (uint8_t)0b11111110;
}
